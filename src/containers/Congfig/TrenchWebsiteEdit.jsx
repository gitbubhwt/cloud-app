...
import { WebChannelGet, WebChannelEdit, } from 'Actions/ConfigAction';
...
export default class TrenchWebsiteEdit extends Component{
    ...
    saveWebsite(){
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                if(this.fileList.length>0){
                    if(this.fileList[0].response){
                        ...
                        let a=this.fileList[0].url;
                        // console.log(a);
                        this.fileList[0].url=a;
                        values.logo=this.fileList[0];
                    }
                }
            ...
            let params={
                channel_id:this.props.routeParams.id,
            ...
            this.button_name=values.button_name;
            this.button_icon=values.button_icon;
            this.button_style=values.button_style;
            // console.log("values.logo.url",values.logo.url);
            // console.log("this.fileList",this.fileList);
            if(this.fileList.length>0&&!values.logo.url){
                notification.warning({
                    'message': '提示',
            ...
            if(values.logo.url){
                if(values.logo.url.indexOf("http://ektfiles.icsoc.net")==-1) {
                    this.logo = "http://ektfiles.icsoc.net" + values.logo.url;
                }else{
                    this.logo=values.logo.url;
                }
            }else{
                this.logo=""
            }
            self.props.dispatch(WebChannelEdit(params))
            }
        });
    ...
    componentWillMount() {
        ...
        this.props.dispatch(WebChannelGet(this.props.routeParams.id))
    }
    componentDidMount(){
    ...
    render(){
        ...
        if(this.props.StaffReducer&&this.props.StaffReducer.webChannelData&&this.firstData){
            this.webChannelData=this.props.StaffReducer.webChannelData;
            this.channel_style=JSON.parse(this.webChannelData.channel_style);
            this.short_name=this.channel_style.short_name;
            this.color=this.channel_style.color;
            this.btnColor=this.channel_style.color;
            this.logoShape=this.channel_style.logo_shape;
            if(this.logoShape==2){
               this.borderRadius="0";
            }else{
                this.borderRadius="50%";
            }
            this.button_name=this.channel_style.button_name;
            this.button_icon=this.channel_style.button_icon;
            this.button_style=this.channel_style.button_style;
            if(this.channel_style.logo.url&&this.channel_style.logo.url.indexOf("http://ektfiles.icsoc.net")==-1){
                this.channel_style.logo.url="http://ektfiles.icsoc.net"+this.channel_style.logo.url;
                this.logo=this.channel_style.logo.url;
             }else{
                this.channel_style.logo.url=this.channel_style.logo.url;
                this.logo=this.channel_style.logo.url;
            }
            if(this.channel_style.logo&&this.channel_style.logo.url){
                this.fileList=[this.channel_style.logo];
            }else{
                this.fileList=[]
            }
                    ...
                    <Form>
                        <Row>
                            <Col span={12}>
                                <Row>
                                    <FormItem label="来源名称" {...formItemLayout}>
                                        {getFieldDecorator('source_name',{
                                            initialValue:this.webChannelData.source_name,
                                            rules: [{
                                    ...
                                    <FormItem label="企业简称" {...formItemLayout}>
                                        {getFieldDecorator('short_name',{
                                            initialValue:this.webChannelData.short_name,
                                            rules: [{
                                        ...
                                        <Col span={12} style={{"lineHeight":"31px"}}>
                                            {/*<FormItem label="按钮样式" {...formItemLayoutLogo}>*/}
                                                {getFieldDecorator('logo_shape',{
                                                    initialValue:this.logoShape?this.logoShape:1,
                                    ...
                                    <FormItem label="按钮样式" {...formItemLayout}>
                                        {getFieldDecorator('button_style',{
                                            initialValue:this.channel_style.button_style,
                                    ...
                                    <FormItem label="按钮图标" {...formItemLayout}>
                                        {getFieldDecorator('button_icon',{
                                            initialValue:this.channel_style.button_icon,
                                    ...
                                    <Row>
                                        ...
                                        {getFieldDecorator('button_name',{
                                            initialValue:this.channel_style.button_name,
                                            rules: [{
                                ...
                                <Row>
                                    <Col span={2} />
                                    <Col span={10}><Button type="primary" onClick={this.saveWebsite.bind(this)}>{/*<i className="fa fa-check" ></i>*/}&nbsp;保存</Button></Col>
                                </Row>
                            </Col>
                ...
                <Card title="在网站中嵌入代码">
                    ...
                    <Row>
                        <Col span={15}  style={{"position":"relative"}}>
                            <TextArea rows={2} id="biao1" value={"<script src="+"\""+this.webChannelData.web_channel_static_url+"\""+"></script>"}/>
                            <Button onClick={this.copyUrl2.bind(this,1)} className="copy">复制</Button>
                        </Col>
                    </Row>
                    {/*<Row className="copyLine">https协议代码块</Row>
                    <Row>
                        <Col span={15}  style={{"position":"relative"}}>
                            <TextArea rows={2} id="biao3" value={"<script src="+"\""+httpsStatis+"\""+"></script>"}/>
                            <Button onClick={this.copyUrl2.bind(this,3)} className="copy">复制</Button>
                        </Col>
                    </Row>*/}
                    <Row>
                    ...
                    <Row>
                        <Col span={15} style={{"position":"relative"}}>
                            <TextArea rows={2} id="biao2" value={this.webChannelData.web_channel_sever_url+"&init=1"}/>
                            <Button onClick={this.copyUrl2.bind(this,2)} className="copy">复制</Button>
                        </Col>
                    </Row>
                    {/*<Row className="copyLine">https协议代码块</Row>
                    <Row>
                        <Col span={15} style={{"position":"relative"}}>
                            <TextArea rows={2} id="biao4" value={httpsServer+"&init=1"}/>
                            <Button onClick={this.copyUrl2.bind(this,4)} className="copy">复制</Button>
                        </Col>
                    </Row>*/}
                    <Row>
...
TrenchWebsiteEdit.contextTypes = {
    router: PropTypes.object,
};

TrenchWebsiteEdit = Form.create()(TrenchWebsiteEdit);
const mapStateToProps = TrenchWebsiteEdit => TrenchWebsiteEdit;
module.exports = connect(mapStateToProps)(TrenchWebsiteEdit);