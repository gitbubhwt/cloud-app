...
import {
    ...
    TimePicker,
    DatePicker,
    Alert, Table, Popconfirm, notification
} from 'antd';
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                for (let i = 0; i < self.dataSource.length; i++) {
                    ...
                    rest.push({ name: self.dataSource[i].name, from: self.dataSource[i].from_to[0], to: self.dataSource[i].from_to[1] });
                }
                const wh_settings = {
                    working: working,
                    rest: rest
            ...
            if (i == 0) {
                ...
                this.workTimeDomSeven.push(<div key={`a+${i}`}><Col span={3}> </Col><Row>
                    ...
                    <Col span={3}><TimePicker defaultValue={moment(this.working[0][i].to, format)} format={format}
                        onChange={this.timeChange.bind(this, `seven_2_${i}`)} /></Col>
                    <Col span={1} className="iconFont" style={{ "textAlign": "center" }}><i className="fa fa-minus-circle" aria-hidden="true" onClick={this.deleteTime.bind(this, "seven", i)}> </i></Col>
                </Row></div>)
            }
        }
        let column = [{
